...
function g_sigatom(f::Base.Callable) # calls f, were f never throws (but this function may throw)
    global g_sigatom_flag, gtk_stack, gtk_doatomic
    prev = g_sigatom_flag
    stk = gtk_stack
    ct = current_task()
    if gtk_yielded
        @assert gtk_stack !== nothing && gtk_stack != ct && !prev
        push!(gtk_doatomic, (f, ct))
        return wait()
    end
...
function g_siginterruptible(f::Base.Callable, cb) # calls f (which may throw), but this function never throws
    global g_sigatom_flag, gtk_stack
    prev = g_sigatom_flag
...
function g_yield(data)
    global gtk_yielded, gtk_doatomic
    while true
        gtk_yielded = true
        g_siginterruptible(yield, yield)
        gtk_yielded = false

        if isempty(gtk_doatomic)
            return int32(true)
        else
            f, t = pop!(gtk_doatomic)
            ret = nothing
            iserror = false
            try
                ret = f()
